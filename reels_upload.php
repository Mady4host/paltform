...
<head>
<meta charset="UTF-8">
<title>رفع (Reel / Story)</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.rtl.min.css" rel="stylesheet">
<style>
body{background:#f1f4f8;font-family:Tahoma,Arial;}
h2{font-weight:600;color:#0d4e96;}
.auto-hide{transition:opacity .7s;}
.pages-wrapper{background:#fff;border:1px solid #d6e1ee;border-radius:10px;padding:10px;max-height:260px;overflow-y:auto;}
.page-row{display:flex;align-items:center;gap:10px;padding:6px 4px;border-bottom:1px solid #edf1f5;font-size:13px;cursor:pointer;}
.page-row:last-child{border-bottom:none;}
.page-row:hover{background:#f5faff;}
.page-row img{width:40px;height:40px;border-radius:50%;object-fit:cover;border:1px solid #c4d2e2;}
#pageSearch{width:220px;display:inline-block;}
.hashtag-btn{background:#eef2fa;border:1px solid #c5d3ec;border-radius:16px;padding:2px 10px;font-size:13px;cursor:pointer;margin:2px;color:#194b92;}
.hashtag-btn:hover{background:#e2ebf9;}
.selected-tags-box{background:#fff;border:1px solid #cfd9e7;border-radius:10px;padding:10px;min-height:48px;font-size:13px;display:flex;flex-wrap:wrap;gap:6px;}
.selected-tags-box span{background:#194b92;color:#fff;padding:3px 10px;border-radius:14px;display:inline-flex;align-items:center;gap:6px;}
.selected-tags-box span i{cursor:pointer;font-style:normal;font-weight:bold;}
#videoDrop{border:2px dashed #a9c1d9;background:#fff;border-radius:10px;padding:24px;text-align:center;cursor:pointer;transition:.25s;}
#videoDrop:hover{background:#f5faff;}
.video-card{background:#fff;border:1px solid #d9e3ef;border-radius:12px;padding:14px;margin-top:20px;position:relative;display:flex;gap:18px;}
.video-thumb video{width:220px;height:390px;object-fit:cover;background:#000;border-radius:10px;}
.remove-video{position:absolute;top:6px;left:6px;font-size:13px;background:#dc3545;color:#fff;border:1px solid #b52d3a;border-radius:22px;padding:5px 15px;cursor:pointer;}
.remove-video:hover{background:#bb2332;}
.cover-actions{display:flex;gap:10px;margin-top:10px;}
.cover-actions .btn{flex:1;}
.cover-thumbs{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;}
.cover-thumbs .thumb{width:70px;height:110px;border:2px solid transparent;border-radius:6px;overflow:hidden;cursor:pointer;background:#eee;}
.cover-thumbs .thumb img{width:100%;height:100%;object-fit:cover;}
.cover-thumbs .thumb.selected{border-color:#0d6efd;box-shadow:0 0 0 2px rgba(13,110,253,.35);}
.comment-block{background:#f9fbfe;border:1px solid #d4e2f1;border-radius:10px;padding:10px;margin-top:10px;}
...
.footer-actions{margin-top:28px;display:flex;gap:14px;}
.story-note{font-size:12px;color:#b35c00;margin-top:4px;line-height:1.5;display:none;}
.story-photo-box{display:none;background:#fff;border:1px solid #d6e1ee;border-radius:10px;padding:14px;margin-bottom:18px;}
#contentTypeWrapper{background:#fff;border:1px solid #cfd9e7;border-radius:10px;padding:14px;display:flex;align-items:center;justify-content:space-between;gap:15px;}
...
?>
<div class="container" style="max-width:1300px;padding-top:25px;">
    <h2 class="text-center mb-4">رفع (Reel / Story)</h2>
    ...
    <form id="reelsForm" method="post" action="<?= site_url('reels/process_upload') ?>" enctype="multipart/form-data">
        ...
        <div class="mb-4" id="contentTypeWrapper">
            <div class="fw-bold" style="font-size:15px;">نوع المحتوى:</div>
            <div class="dropdown">
        ...
        <!-- صورة ستوري -->
        <div id="storyPhotoWrapper" class="story-photo-box">
        ...
        <div class="mb-4">
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-2 gap-2">
                <div class="fw-bold">اختر الصفحات:</div>
        ...
        <div class="mb-4" id="globalDescBlock">
            <div class="d-flex justify-content-between align-items-center">
                <label class="form-label fw-bold mb-0">وصف عام (يستخدم لو وصف الفيديو فارغ):</label>
                <div class="d-flex gap-2" id="hashtagButtons">
                    <button type="button" id="btnGenerateAllTags" class="btn btn-sm btn-outline-primary">توليد هاشتجات اليوم</button>
                    <button type="button" id="btnClearTags" class="btn btn-sm btn-outline-danger">حذف كل الهاشتاجات</button>
                </div>
            </div>
        ...
        <div class="footer-actions">
            <button type="submit" class="btn btn-primary flex-grow-1">تنفيذ</button>
            <a href="<?= site_url('reels/list') ?>" class="btn btn-secondary">رجوع</a>
        </div>
    </form>
</div>
...
function toggleMediaTypeUI(){
    ...
    storyPhotoWrapper.style.display = isStoryPhoto ? 'block':'none';
...
document.getElementById('reelsForm').addEventListener('submit',function(e){
    ...
    if(mediaType==='story_photo'){
        const f = this.querySelector('input[name="story_photo_file"]');
        if(!f || !f.files.length){ alert('اختر صورة للستوري'); return; }
        basicSubmit(this);
        return;
    }
    if(mediaType==='story_video' || mediaType==='reel'){
        ...
        videoItems.forEach((item,oldIdx)=>{
            ...
            if(mediaType==='reel'){
                ...
                document.querySelectorAll('[name^="comments['+oldIdx+']"]').forEach(inp=>{
                    const newName=inp.name.replace('comments['+oldIdx+']','comments['+newIdx+']');
                    fd.append(newName, inp.value);
                });
                if(item.cover){
...
function uploadWithProgress(fd,actionUrl){
    ...
    xhr.onreadystatechange=()=>{
        if(xhr.readyState===4){
            ...
            } else alert('خطأ أثناء الإرسال (HTTP '+xhr.status+')');
        }
    };
    xhr.send(fd);
}
